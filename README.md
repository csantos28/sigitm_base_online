# Pipeline de Automação SIGITM - Base Online


## 🎯 Objetivo

Esta pipeline automatiza a extração, transformação e carga (ETL) de dados de bilhetes em status "Online" do sistema SIGITM da Vivo, especificamente para a consulta CONSULTA_TLP_PCP_CS, armazenando-os em um banco de dados PostgreSQL para monitoramento em tempo real e análise de desempenho.
...
### 1. `main_base_online.py` - **Orquestrador Principal**
...
- Implementa cache de status para performance (5 segundos)
...
- Navegação até a consulta específica "CONSULTA_TLP_PCP_CS"
- Ajuste automático de datas (sempre busca dados da data atual)
- Verificação inteligente: só altera data se necessário
...
- Controle de tempo de carregamento com coroutines paralelas
...
- Extrai metadados do nome do arquivo (data/hora da extração)
- Adiciona colunas de carga (dt_carga, dthr_carga)
- Mapeamento de colunas para nomes padronizados (snake_case)
- Tratamento de datas:
  - Conversão para formato ISO (YYYY-MM-DD HH:MM:SS)
- Limpeza de dados:
  - Tratamento de valores nulos (NaN, NaT, "None", "")
  - Normalização de IDs (inteiros seguros)
  - Preservação de todas as colunas relevantes
- Validação de tipos e consistência

#### **Diferencial**: Extrai timestamp do nome do arquivo para rastreabilidade completa
...
3.  Fase 2 - Extração
    ...
    - Verificação/ajuste da data de baixa (só altera se necessário)
...
4. Fase 3 - Transformação
    ...
    - Mapeamento e adição de colunas de carga (dt_carga, dthr_carga)
